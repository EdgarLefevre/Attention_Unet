...
def unet(input_shape, filters=16, drop_r=0.2, attention=False):
    ...
    u4 = utils_model.block_up(o, [c4], filters=8 * filters, drop=drop_r, name="block5", attention=attention)
    u3 = utils_model.block_up(u4, [c3], filters=4 * filters, drop=drop_r, name="block6", attention=attention)
    u2 = utils_model.block_up(u3, [c2], filters=2 * filters, drop=drop_r, name="block7", attention=attention)
